...
/**
 * 退款超时死信消息消费者
 * 监听进入死信队列的退款超时消息，记录并告警
 */
@Slf4j
@Component
@RequiredArgsConstructor
@RocketMQMessageListener(
        topic = "%DLQ%refund-timeout-consumer-group",
        consumerGroup = "dlq-refund-timeout-consumer-group",
...
public class DlqRefundTimeoutConsumer implements RocketMQListener<MessageExt> {
    
    private final MqAlertService mqAlertService;
    
    @Override
    public void onMessage(MessageExt message) {
        log.error("【严重】退款超时消息进入死信队列, msgId: {}, keys: {}, body: {}", 
                message.getMsgId(), 
                message.getKeys(),
        ...
        mqAlertService.recordAndAlert(
                message, 
                "refund-timeout-consumer-group",
                "REFUND_TIMEOUT",
                "退款超时处理失败，重试16次后进入死信队列，需要人工介入处理"
        );
